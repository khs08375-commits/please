...
// 동물상별 특징 데이터 (모델의 실제 클래스명에 맞춰 매핑)
...
tabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (target !== 'animal-tab' && webcam) stopWebcam();
    });
});
    ...
    for (let i = 0; i < maxPredictions; i++) {
        ...
        wrapper.innerHTML = `
            <div class="result-label">
                <span class="class-name">로딩 중...</span>
...
async function predict(inputElement) {
    ...
    for (let i = 0; i < maxPredictions; i++) {
        const className = prediction[i].className; // 모델의 실제 클래스명 사용
    ...
    // 특징 표시 (매핑된 데이터가 있을 경우)
    const featureKey = Object.keys(animalFeatures).find(key => topAnimal.name.includes(key));
    if (featureKey) {
        resultTitle.textContent = featureKey + " (" + (topAnimal.prob * 100).toFixed(0) + "% 일치)";
        resultDescription.textContent = animalFeatures[featureKey].description;
        resultMessageContainer.style.display = 'block';
    } else {
        resultTitle.textContent = topAnimal.name + " (" + (topAnimal.prob * 100).toFixed(0) + "% 일치)";
        resultDescription.textContent = "분석 결과 " + topAnimal.name + "와 가장 닮으셨네요!";
        resultMessageContainer.style.display = 'block';
    }
}
